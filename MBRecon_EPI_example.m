...
%%
NSlc = 84;% number of slices
MB = 4;% multiband-factor
NSlc_mb = NSlc/MB;
numCha = 32; % number of channels
NMeas = 116; % number of measurements
nADC = 116; % number of samples in kx
nLin = 57; % number of phase encoding lines
twix_obj.image.flagRemoveOS = true;
twix_obj.image.flagRampSampRegrid = true;
image_raws = squeeze(twix_obj.image.unsorted()); % read the raw k-space data for image measurements
...
%% save raw data for each image measurement to save processing memory
for m = 1:NMeas
...
clear image_raws
%% read phase correction calibration scans
