...
%%%%% Parse varargin %%%%%
arg.bReadOffline    = true;
% arg.bReadSliceAccelRefScan = true;
% arg.bReadSliceAccelPhscor = true;
...
for s=1:NScans
    ...
    twix_obj{s}.offline       = twix_map_obj(arg,'offline',filename,version);
%     twix_obj{s}.sgrefscan     = twix_map_obj(arg,'sgrefscan',filename,version);
%     twix_obj{s}.sgrefscanPC   = twix_map_obj(arg,'sgrefscanPC',filename,version);
    ...
    if arg.bReadOffline
        clear tmpMdh
        isCurrScan = logical( mask.MDH_OFFLINE);
        for f = fieldnames(mdh).'
            tmpMdh.(f{1}) = mdh.(f{1})( isCurrScan, : );
        end
        twix_obj{s}.offline.readMDH( tmpMdh, filePos(isCurrScan) );
    end
    ...
    for scan = {'offline', 'image','noise', 'phasecor', 'phasestab', ...
...
% data_uint64     = typecast( reshape(mdh_blob(1:76,:),  [],1), 'uint64');
...
% mdh.aulEvalInfoMask            = data_uint64(:,6:7);
...
mask.MDH_OFFLINE           = min(bitand(evalInfoMask1, 2^4), 1);
...
% mask.MDH_SLICE_ACCEL_REFSCAN = min(bitand(mdh.aulEvalInfoMask(2), 2^38),1);
% mask.MDH_SLICE_ACCEL_PHASCOR = min(bitand(mdh.aulEvalInfoMask(2), 2^39),1);
...
noImaScan = ( mask.MDH_ACQEND           | mask.MDH_RTFEEDBACK   | mask.MDH_HPFEEDBACK    ...
            ...
            | mask.MDH_REFPHASESTABSCAN | mask.MDH_SYNCDATA | mask.MDH_OFFLINE);
